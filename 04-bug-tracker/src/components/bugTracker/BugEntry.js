...
const BugEntry = (props) => {
  const { bug, icon, reporter, createdAt, status, assignee, severity } = props;
  ...
  return (
    <ul className="bug__entry-list">
      <li className="bug__entry-item">{bug}</li>
      <li className="bug__entry-item">{icon && icon}</li>
      <li className="bug__entry-item">{reporter}</li>
      <li className="bug__entry-item">{createdAt}</li>
